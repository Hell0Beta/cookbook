...
| Quota UX (§5) | `components/llm-quota.tsx`, `CHAT_QUOTA_NOTICE` | `src/components/QuotaNotice.tsx` | M9 |
| Bento system | `components/bento.tsx`, tokens in `app/globals.css` + `cookbook ui idea/heirloom_kitchen/DESIGN.md` | `src/lib/theme.ts`, `src/components/` | M4 |
...
- [ ] `OfflineBanner` in `app/_layout.tsx` — status text currently lives on Home only *(mobile)* → folds into M4's design pass
...
- [ ] Token port: `src/lib/theme.ts` gains the full Heirloom Kitchen palette from `app/globals.css` + `cookbook ui idea/heirloom_kitchen/DESIGN.md` (colors, radii, type scale) *(mobile)*
- [ ] `src/components/` bento primitives from `components/bento.tsx` — cell with image/text/action regions *(mobile)*
- [ ] Recipe list on Home from `components/recipes-browser.tsx` + card cells in `components/dashboard-grid.tsx`: cover (`RecipeCover` → `apiAsset()`), title, meta, swipe actions, pull-to-refresh (replaces M3's plain rows) *(mobile)*
...
- [ ] Images: `src/lib/images.ts` porting `apiAsset()` (`/images/*` is API-origin, prefix `NEXT_PUBLIC_API_URL` equivalent) + hero downloads via `expo-file-system` *(mobile)*
- [ ] Stub → full rehydrate from `src/db/repositories/recipes.ts`; "needs internet" state for stub rows offline (design.md §5) *(mobile)*
- [ ] Deep links `cookbook://recipe/:id` + `/recipe/[id]` route params *(mobile)*
- [ ] Carryover: `OfflineBanner` in root layout; first-run sync progress *(mobile)*

## M5 — Cooking mode
Development.md §7 ("Timer Tool" / "Shake-to-Advance"), design.md §3.3.2–3.3.3.
- [ ] Read-mode blocks + servings stepper — reuse the M4 read components + shared scaling (§8) *(mobile)*
- [ ] Timers `src/stores/timers.ts` from `components/timer/timer-store.ts`: re-key to absolute timestamps (background-proof), `expo-notifications` scheduled triggers, vibration; Android 13+ permission on first start *(mobile)*
...
- [ ] Planner `app/(tabs)/planner.tsx` from `app/planner/planner-client.tsx`: week strip, slot cells, entry CRUD via repositories; slot resolution from shared (§8/§11) *(mobile)*
- [ ] Quick add sheet from `components/quick-add-modal.tsx` — parameterized by `recipe_id`, shared `resolveQuickAddTarget` preview line *(mobile)*
- [ ] Grocery `app/(tabs)/grocery.tsx` from `app/grocery-list/grocery-list-client.tsx`: category groups, check-off, recategorize, manual add — aggregation math from shared (§8) *(mobile)*

## M8 — Search + filters + favorites + Discover + diet profile
- [ ] Server search from `app/search/search-client.tsx`: debounced query, pagination envelope, tag tree + ingredient matching modes *(mobile)*
...
- [ ] Diet profile from `app/profile/diet-profile-form.tsx`: diet/allergen/cuisine chips, excluded ingredients *(mobile)*
...
- [ ] YouTube import from `components/youtube-import-panel.tsx` — review-before-save, `no_recipe_content` nudge, free-tier wait state *(mobile)*
- [ ] Public API import from `components/import-panel.tsx` + the create flow in `app/recipes/new/page.tsx` *(mobile)*
- [ ] `llm_quota_exceeded` UX (design.md §5) from `components/llm-quota.tsx` — disable AI features only, never block core flows *(mobile)*
