...
- [ ] Browser smoke: bar render/dock vs timer pill, tap-cycle + drag resize, record-from-bar, instant vs review, always-on loop (turn end on silence, no self-hearing during replies, interrupt → re-arm), settings persistence across reload *(frontend — needs a live browser session)*
