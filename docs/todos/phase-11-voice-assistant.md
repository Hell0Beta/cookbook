...
## Assistant bar + resizable sheet + hands-free modes (owner request, 2026-09-12; design.md §3.3.4, development.md §14.1)
- [x] Collapsed form is the assistant BAR (replaces the round mic button): expand/resize control (left), status/last-reply snippet (middle, tap opens the sheet), record button, settings gear — `chat-bar.tsx` *(frontend)*
- [x] Resizable sheet: tap cycles bar → sheet (~55vh) → tall sheet (~85vh) → bar; press-and-drag (bar control or sheet top edge) resizes continuously, clamped 30–90vh; height + form persist (`cookbook:chatPanelHeight`) — `resize-handle.tsx` tap-vs-drag pointer control *(frontend)*
- [x] Send-on-stop setting: "instant" (default — a finished recording sends immediately, reply spoken + shown, bar snippet while collapsed) vs "review" (transcript lands in the editable input, the old pre-send affordance) *(frontend)*
- [x] Voice settings popover (gear on the bar + sheet header): mic mode (Standard / Always-on) + send-on-stop, persisted (`cookbook:chatSettings`) — `chat-settings.tsx` *(frontend)*
- [x] Always-on mic mode: AnalyserNode RMS sampling → `lib/vad.ts` `TurnSegmenter` (pure, unit-tested — `tests/vad.test.ts`); silence ends a turn → transcribe → send; mic off while the assistant speaks, re-arms via `speak(text, onEnd)` completion callback; noise-only captures discarded with a bar notice; STT/mic errors halt the loop cleanly *(frontend)*
- [x] Recording from the bar bootstraps the session + TTS warm-up without opening the sheet; instant voice turns arriving pre-session are queued and flushed on resolve *(frontend)*
- [x] **Bugfix round (owner request, 2026-09-12)**: sheet drag-by-header + bar drag-by-middle (viewport-clamped, session-scoped positions); cancel (X) next to record discards recording/transcription with no send; settings popover closes on true outside clicks (gear-tap toggles instead of close-then-reopen); TTS is Kokoro-only — no SpeechSynthesis fallback, utterances queue while the model loads (`af_heart`); mic-access pre-check explains the secure-context (https/localhost) requirement where no prompt can ever appear *(frontend)*
- [ ] Browser smoke: bar render/dock vs timer pill, tap-cycle + drag resize, record-from-bar, instant vs review, always-on loop (turn end on silence, no self-hearing during replies, interrupt → re-arm), settings persistence across reload, drag-to-move both forms, cancel mid-recording/transcription, popover outside-click, Kokoro-only voice (incl. first-reply-after-load), mic prompt on a secure origin + the explanatory error on plain http *(frontend — needs a live browser session)*
- [ ] Always-on VAD tuning on a real device (threshold/silence constants are desktop-informed defaults) *(frontend — needs the target phone)*
...
## Verification log
- 2026-09-12: assistant bar + resizable sheet + hands-free modes — typecheck clean; 23/23 web tests (incl. 7 new `TurnSegmenter` VAD tests in `tests/vad.test.ts`); production build compiles + generates all pages, failing only at the pre-existing Windows standalone-symlink trace step (same EPERM as 2026-09-03 — real packaging in the Linux Docker build). **Still open: browser smoke (bar/drag-resize/record-from-bar/instant-vs-review/always-on loop) + on-device VAD tuning.**
