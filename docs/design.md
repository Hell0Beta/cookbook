...
A collapsible conversational panel scoped to the Recipe Reader in **read (cooking) mode** — the hands-free companion to the Timer Tool and Shake-to-Advance. Speech is the primary input; a text field is always present as the equal fallback. **Fully local speech**: transcription runs on-device (development.md §14), and spoken replies use the on-device Kokoro voice — the browser's platform speech is never used (replies queue until the model is ready; if it fails, they are shown but silent). No speech audio ever leaves the device.
...
- Contents, left to right: **expand/resize control** (tap cycles bar → sheet → tall sheet → bar; drag up grows the sheet with its top edge tracking the pointer — the panel always grows upward from the bottom dock), a **middle area** (live status — animated waveform while listening, spinner while transcribing/thinking — or the latest reply's first line truncated; tapping it opens the sheet, dragging it moves the bar anywhere on screen), the **cancel button** (always present next to record; enabled while the pipeline has something in flight — recording, transcribing, or the initial speech-model load — and discards it, sending nothing), the **record button** (tap to start/stop recording without expanding), and the **settings gear** (far right; opens the voice settings popover; clicking anywhere outside it collapses it).
- The bar docks bottom-right by default and can be dragged to any screen edge; the expanded sheet is likewise movable — drag its header to carry it out of the way of the recipe behind it. Positions are clamped to the viewport and last for the cooking session.
...
- **Sizing & placement:** tap the expand/resize control to cycle bar → half sheet (~55vh) → tall sheet (~85vh) → bar; drag the control up (or the sheet's top-edge strip) to resize continuously (clamped ~30–90vh) — the sheet's top edge follows the pointer and its bottom edge stays at the dock, so it always grows upward. The custom height and the collapsed/expanded form persist across opens (per-device). Drag the sheet's header to move it anywhere on screen.
