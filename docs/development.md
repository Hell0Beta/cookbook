...
- **TTS:** primary and ONLY engine is **Kokoro-82M** (Apache-licensed, 82 M params) via `kokoro-js` in a dedicated Web Worker — natural speech, consistent across devices, fully local (q8 ONNX ~92 MB + voice embeddings, vendored under `apps/web/public/models/Kokoro-82M-v1.0-ONNX/` by `pnpm vendor:tts`, which also runs on `postinstall` so models are fetched at install/build time). Kokoro-js hardcodes a HuggingFace voice URL — `tts-worker.ts` pre-populates the `"kokoro-voices"` Cache API cache from the app's own origin so that fetch never fires (§0). The phonemizer dependency (espeak-ng) is asm.js embedded in the bundle — no network. **No platform-voice fallback (owner request 2026-09-12):** replies queue while the model loads and flush when ready; if loading fails, replies are silent and the transcript carries them. Default voice `af_heart` (female). `speak(text, onEnd?)` fires `onEnd` exactly once when playback finishes, fails, or is cancelled — the Always-on mic mode sequences on it (below).
- **Microphone access:** `getUserMedia` exists only in secure contexts — over plain http (e.g. a LAN IP) the API is absent and **no permission prompt can ever appear**, on mobile or desktop (same secure-context gating as shake-to-advance). The capture layer detects this and surfaces why ("open the app over https or localhost") instead of a generic denial; on a secure origin the record tap triggers the browser's permission prompt normally.
- **Send-on-stop (design.md §3.3.4 "After recording"):** a finished recording either sends immediately or lands in the editable input for correction, per the persisted user setting (`cookbook:chatSettings`). Always-on turns always send immediately.
- **Always-on mic mode (design.md §3.3.4 "Voice modes"):** the capture loop stays open across turns. An `AnalyserNode` taps the same `MediaStream` (no extra audio path) and samples RMS every 250 ms into `TurnSegmenter` (`apps/web/src/lib/vad.ts`, pure + unit-tested): speech ≥ 300 ms starts a turn, ~1.2 s trailing silence ends it, 30 s is the safety cutoff, blips under the minimum are discarded as noise (the bar shows a "didn't catch that" notice and the loop re-arms). Turn end → recorder stops → the existing transcribe path → the turn sends → the reply is spoken with `onEnd` → the mic re-arms. The mic is deliberately **off while the assistant speaks** (it would hear itself); a tap on the record button mid-speech is the interruption path. STT/mic errors halt the loop cleanly (no crash-re-arm cycle).
